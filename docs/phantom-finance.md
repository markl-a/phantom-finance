...
| --- | --- | --- | --- |
| 讓 recurring 偵測變成可審狀態 | `recurring.json` 持久化 + 每筆 review 狀態(new/reviewed/ignored)+ `recurring review`/`recurring list` | z13(Win)編排把關;寫 codex/claude,審 codex+agy | 純本機、無新依賴;低風險。已有開發分支 |
...
| (僅在真有需求時)PTA 互通 | 可選 Beancount/hledger **匯出**(檔案格式級,非 import) | acer/ayaneo on-demand | 🧭 候選;Beancount/hledger 是 GPL,只能**格式級互通**,import 會 GPL 耦合。多數單人情境永遠用不到 |
